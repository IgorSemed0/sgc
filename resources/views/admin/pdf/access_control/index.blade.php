...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Controle de Acesso</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="insignia">
            ...
            <div class="textos-cabecalho">
                ...
                <p>Relatório de Controle de Acesso</p>
            </div>
        </div>
    </div>
    <hr>
    ...
    if (!empty($_POST['mes_inicio']) && !empty($_POST['mes_fim'])) {
        $mesInicio = nomeMesPortugues($_POST['mes_inicio']);
        $mesFim = nomeMesPortugues($_POST['mes_fim']);
        $descricaoFiltros[] = "Intervalo de Meses: " . $mesInicio . " a " . $mesFim;
    }
    if (!empty($_POST['ano'])) {
        $descricaoFiltros[] = "Ano: " . $_POST['ano'];
    }
    if (!empty($_POST['tipo_pessoa'])) {
        $descricaoFiltros[] = "Tipo de Pessoa: " . ucfirst($_POST['tipo_pessoa']);
    }
    if (!empty($_POST['tipo'])) {
        $descricaoFiltros[] = "Tipo de Acesso: " . $_POST['tipo'];
    }
                    ...
                    <tr>
                        ...
                        <td>
                            @if($acesso->tipo_pessoa === 'morador')
                                {{ optional($acesso->morador)->primeiro_nome . ' ' . optional($acesso->morador)->ultimo_nome ?? '—' }}
                            @elseif($acesso->tipo_pessoa === 'funcionario')
                                {{ optional($acesso->funcionario)->primeiro_nome . ' ' . optional($acesso->funcionario)->ultimo_nome ?? '—' }}
                            @elseif($acesso->tipo_pessoa === 'visitante')
                                {{ optional($acesso->visitante)->primeiro_nome . ' ' . optional($acesso->visitante)->ultimo_nome ?? '—' }}
                            @endif
                        </td>
